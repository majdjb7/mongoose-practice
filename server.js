...
console.log("\nBooks: Ex3\n")
Book.find({genres: "Fiction"}).skip(2).limit(3).exec(function(err, fictionBooks) {
...
console.log("\People: Ex1\n")
Person.find({$and:
            [
              { height: {$gt: 180} },
              { salary: { $gt: 30000 } }
            ]
            }, function(err, RichAndTall) {
  console.log(RichAndTall)
})


//2. Find all the people who are tall (>180) OR rich (>30000)
console.log("\People: Ex2\n")
Person.find({$or:
            [
              { height: {$gt: 180} },
              { salary: { $gt: 30000 } }
...
Person.find({$and:
              ...
              weight: {$lt: 70}}, function(err, Skinny) {
  console.log(Skinny)
})
//4. Find people who have at least 1 kid with grey hair
console.log("\People: Ex4\n")
Person.find({kids:{$elemMatch:{hair:"grey"}}}).exec(function(err, peopleWithGrayKids) {
  for (p in peopleWithGrayKids) {
    let person = peopleWithGrayKids[p]
    console.log("Person", peopleWithGrayKids, "has kids: \n", person.kids)
  }
})

//5. Find all the people who have at least one overweight kid, and are overweight themselves (>100)
console.log("\People: Ex5\n")
Person.find({$and:
          [
          {weight: {$gt: 100}},
          {kids:{$elemMatch:{weight:{"$gt": 100}}}}
          ]}).exec(function(err, overWeightFamily) {
            for(p in overWeightFamily){
              var person = overWeightFamily[p];
